...
const NIFTY100 = (process.env.NIFTY100 || 'HAL,INFY,TCS,RELIANCE,ITC,HDFCBANK,ICICIBANK,SBIN,LT,BAJFINANCE').split(',').map(s => s.trim()).filter(Boolean);
...
app.get('/auth/zerodha/callback', async (req, res) => {
  try {
    ...
    const session = await kite.generateSession(request_token, checksum);
